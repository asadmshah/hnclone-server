...
constructor(private val dataSource: DataSource) : SessionsDatabase {
    ...
    override fun create(id: Int, expires: LocalDateTime): Session? {
        ...
        return dataSource
                .executeSingle("SELECT * FROM sessions_create($id, '$uuidStr', $expiresDouble);", { it.toSession() })
    }

    override fun read(id: Int): Observable<Session> {
        return dataSource
                .executeObservable("SELECT * FROM sessions_read_id($id);", { it.toSession() })
    }

    override fun read(uuid: String): Session? {
        return dataSource
                .executeSingle("SELECT * FROM sessions_read_uuid('$uuid');", { it.toSession() })
    }

    override fun delete(id: Int): Int {
        return dataSource
                .executeSingle("SELECT * FROM sessions_delete($id);", { it.toInt() }) ?: 0
    }

    override fun delete(uuid: String): Boolean {
        return dataSource
                .executeSingle("SELECT * FROM sessions_delete('$uuid');", { it.toBoolean() }) ?: false
    }
}